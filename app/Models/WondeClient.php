...
class WondeClient extends Model
{
    ...
    public function getTeachersIds()
    {
        ...
        foreach ($school
                     ->employees
                     ->all(["employment_details", "classes", ]) as $employee)
        {
            if ($employee
                    ->employment_details
                    ->data->teaching_staff && !empty($employee
                    ->classes
                    ->data))
            {
    ...
    public function getTeachersClassSchedule($teachersId)
    {
                     ...
                     ->all(["employment_details", "classes", ]) as $employee)
        {
            ...
            {
                array_push($classeDetailsArray, $employee
                    ->classes
                    ->data);
                break;
            }
    ...
    public function getClassRegister($class_name)
    {
        ...
        foreach ($school
                     ->classes
                     ->all(["students", ], ['class_name' => $class_name]) as $class)
        {
            $studentsArrayForClass = $class
                ->students->data;
        }
    ...
    public function lessonPeriodSchedule($classId)
    {
        ...
        foreach ($school
                     ->lessons
                     ->all(["class", "period", ], ['lessons_start_after' => $this->getCurrentDateTime() ]) as $lessons)
        {
            if ($lessons
                    ->class
                    ->data->id == $classId)
    ...
    public function displayTeachers($employees)
    {
        ...
        foreach ($employees as $id => $employee)
        {
            ...
            echo '<p><strong>ID:</strong> ' . $id . ' | ' . '<strong>Forename:</strong> ' . $employee['forename'] . ' | ' . '<strong>Surname:</strong> ' . $employee['surname'] . '</p>';
            echo '</div>';
        }
        echo '</div>';
        echo '</body>';
